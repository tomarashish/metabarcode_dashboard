  ...
  function exports(_selection) {
    _selection.each(function (_data) {
      ...
      color.domain(d3.keys(_data[0]).filter(function (key) {
        return (key !== "TaxonomyRank" && key != "TaxonomyName" && key != "total" && key != "values");
      }));
 ...
 */
  ...
  function drawStackBars(_data) {
    ...
    var rect = layer.selectAll("rect")
      ...
      .attr("class", function (d) {
        return (taxonRank + ' ' + d.taxonName.replace(/\s+/g, ''));
      })
      .attr("id", function (d) {
        return (d.x + '-' + +taxonRank + '-' + d.taxonName.replace(/\s+/g, '_'));
      })
    ...
    d3.select("#save" + taxonRank + "Stack").on("click", exportAsImage);
  ...
  function exportAsImage() {
    ...
    var chartName = taxonRank + "_StackedBar";
    var chartId = "#" + taxonRank + "Stack" + ' ' + 'svg';
    ...
    img.onload = function () {
      ctx.drawImage(img, 0, 0); //https://stackoverflow.com/questions/36011605/javascript-upload-image-file-and-resize-to-different-resolution-then-download
